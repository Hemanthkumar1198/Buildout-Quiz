...
public class Quiz {
    ...
    public Quiz(String quizName) {
        ...
        // Note: The statement to be printed should be exactly the same.
        // else
    ...
    // If question is not null then add the question to the list of questions
    ...
    public void attemptQuiz() {
        ...
        for (int i = 0; i < questions.size(); ++i) { // Iterates over the list of Questions
            ...
            int opt = sc.nextInt(); // Takes in the user input and stores it in the integer variable `opt`
            if (opt < 1 || opt > questions.get(i).getChoices().size()) {
                // checks if the choice provided by the user does not lie in the range of the list of choices
                System.out.println("Please choose a number between 1 and " + questions.get(i).getChoices().size());
                // asks the user to choose a number between the given range
                ...
                // else if the option lies within the range, then it is checked whether the provided answer is correct
                // if the provided answer is correct then the finalScore value is incremented by 1
                finalScore++;
            }
        }
